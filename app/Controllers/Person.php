...
class Person extends Controller
{
    ...
    // Validate the input
        if (empty($bday)) {
            return $this->response->setJSON(['success' => false, 'message' => 'Birthday is required']);
        }

    // Check if email already exists for another user
        $existingUser = $model->where('bday', $bday)
        ->where('id !=', $userId)
        ->first();
        ...
        if ($existingUser) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Birthday is already in use by another user.'
            ]);
        }
...
public function delete($id){
    ...
    if ($deleted) {
        $logModel->addLog('Delete person', 'DELETED');
